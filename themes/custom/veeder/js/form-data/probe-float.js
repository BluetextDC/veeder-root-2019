...
*/
   ...
   * [9]=Connection        [10]=Probe Length    [11]=Float Type
   * [12]=Float Size       [13]=Cable Length
  */
  var menuSelections = ["grpGasoline", "prdGs8", "empty", "empty", "empty", 
 ...
 var menus =  `[
  ...
  "dlList":[
    {"ddSelectionId":"grpAviation",
     "ddProductGroupItems": [
      {"ddItem":"Aviation Gasoline", "selected":true, "value":"avAviation", "grayed":false},
    ...
    {"ddSelectionId":"grpDiesel",
     "ddProductGroupItems": [
      ...
      {"ddItem":"Diesel (Fuel Oil #2)", "selected":true, "value":"dsDiesel", "grayed":false},
    ...
    {"ddSelectionId":"grpGasoline",
     "ddProductGroupItems": [
      ...
      {"ddItem":"Regular Unleaded", "selected":true, "value":"gsRegular", "grayed":false},
      {"ddItem":"Gasoline with <=15% ETBE", "selected":true, "value":"gsETBE", "grayed":false},
      {"ddItem":"Gasoline with <=12% TBA", "selected":true, "value":"gsTBA", "grayed":false}
     ]
    },
...
//TBR: console.log(consoleObj);
  ...
  function rebuildSelectList(currMenu) {
    ...
    switch (currMenu) {
      ...
      case "floatType":
        ...
        return;
      case "floatSize":
        reduceFloatType(menuSelections[11]);
        return;
      case "cableLength":
  ...
  function clickMeasurement(v) {
    ...
    if (floatIncludedWithProbe()) {
      document.getElementById("__htmlfltPartNumber").innerHTML = "-- FLOAT INCLUDED WITH PROBE --";
      return (true);
    }
    ...
    if (menuSelections[11] !== "empty")
      ...
      {                           // No, float not selected
        clearFloatActive();       // Reset float list
  ...
  function clearProbePartNumber() {
    document.getElementById("__htmlprbPartNumber").innerHTML = "";
...
//TBA: Should return -1 if nothing found. Caller should check for -1.
  ...
  function buildProbePartNumber(v) {
    ...
    for (let i = 0; i < probeObj[activeProbeIndex].prbLth.length; i++) {
      var node = document.getElementById('__htmlBaseLink');
      ...
      if (probeObj[activeProbeIndex].prbLth[i].msr === v) {
        ...
        document.getElementById("__htmlprbPartNumber").innerHTML =
          `<a href="${htmlBaseLink}/"" target="_blank">${probePartNumber}</a>`;
        return;
      }
    }
  }
  ...
  function clearFloatPartNumber() {
    document.getElementById("__htmlfltPartNumber").innerHTML = "";
  }
  ...
  function buildFloatPartNumber() 
    {
              ...
              switch (floatObj[i].partNumBase)
                {
                  case 846400:
                    ...
                    for (j = 0; j < floatObj[i].fltsz.length; j++)
                      {
                        if (floatObj[i].fltsz[j].name === menuSelections[12])
                          break;
                      }
      ...
      if (found > 0) 
        {
          document.getElementById("__htmlfltPartNumber").innerHTML = 
          `<a href="${htmlBaseLink}/"" target="_blank">${floatPartNumber}</a>`;
        }
      else
        {
          document.getElementById("__htmlfltPartNumber").innerHTML = "-- NO FLOAT AVAILABLE --";
        }
    }
...
/*TBA: Remove for production 
      document.getElementById("actMenuList").innerHTML = JSON.stringify(activeMenuList);
/* */
    ...
    if (menuName === "Product") {
      switch (menuGroup) {
        case "grpAviation":
          activeMenuList = ["avAviation", "avJP8", "avJP10" ];
          break;
        case "grpAlternative":
          activeMenuList = ["afAlcohol", "afDEF", "afETBE", "afEthanol", "afEthylglycol", "afFuel6", "afMTBE", "afMethanol", "afMethylalc", "afPropyglycol", "afUsed", "afWindhshield"];
          break;
        case "grpChemical":
          activeMenuList = ["chAcetone", "chButanol", "chMEKetone", "chEBenzene", "chMIKetone", "chXylene", "chEAcetate", "chBAcetate", "chDichloro", "chPetroNapht", "chIsopropyl", "chDiform", "chNpropyl"];
          break;
        case "grpDiesel":
          activeMenuList = ["dsBiodiesel", "dsDiesel", "dsKerosene"];
          break;
        case "grpGasoline":
          activeMenuList = ["gsGasohol", "gsTBA", "gsMethanol", "gsMTBE", "gsTAME", "gsLeaded", "gsMSpirits", "gsPremium", "gsRegular", "gsETBE"];
          break;
        case "grpLightOil":
          activeMenuList = ["loMotoil", "loGear", "loTrans", "loToluene"];
          break;
        case "grpLPG":
          activeMenuList = ["lpLiqpetrogas", "lpPropane"];
          break;
        case "grpHeavyOil":
          activeMenuList = ["hoBiodiesel", "hoFueloil"];
          break;
        default:
          return;
      }
...
//xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx
  ...
  function buildHTMLFieldNames() {
    var hideDatafiltershow = document.querySelectorAll(".datafiltershow");
    for (var i = 0; i < hideDatafiltershow.length; i++) {
        hideDatafiltershow[i].removeAttribute('style');
    }
    document.getElementById("__htmlfldPartNumber").innerHTML = "Probe Part Number:";
    document.getElementById("__htmlfldDescription").innerHTML = "Probe Description:";
  ...
  function hiddenDataLabel() {
    var hideDatafiltershow = document.querySelectorAll(".datafiltershow");
    for (var i = 0; i < hideDatafiltershow.length; i++) {
        hideDatafiltershow[i].style.display = "none";
    }
...
//TBA: Remove bod of this function for production
    let probeList = [];
    let probeCount = 0;
    for (let i = 0; i < probeObj.length; i++) {
      if (probeObj[i].active === true) {
        probeList[probeCount++] = "Probe Id: " + probeObj[i].probeId;
      }
    }
/*TBA: Remove for production 
